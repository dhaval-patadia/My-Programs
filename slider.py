...
wn.listen()
wn.onkeypress(slider_right, "Right")
wn.onkeypress(slider_left, "Left")
...
while True:
    wn.update()
    # pen.goto(0, 0)
    # pen.write("PRESS a for left and d for right direction")
    ...
    if ((ball.ycor() < -200 and ball.ycor()>-205) and (ball.xcor() > slider.xcor()-50 and ball.xcor() < slider.xcor() +50)):
        ball.sety(-200)
        ball.dy *= -1
        Score += 1
        pen.clear()
